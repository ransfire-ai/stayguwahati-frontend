...
export default function HomePage() {
  ...
  return (
    <div className="bg-slate-50 font-sans min-h-screen flex flex-col justify-between overflow-x-hidden">
      {/* Responsive Navigation */}
      <nav className="sticky top-0 z-[100] bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="h-16 flex items-center justify-between gap-3">
            <button
              ...
              aria-label="Go to StayGuwahati home"
            >
              <span className="text-xl sm:text-2xl">🏠</span>
              <span className="text-lg sm:text-xl font-black text-slate-900 tracking-tight truncate">
            ...
            <div className="hidden md:flex items-center gap-5 lg:gap-6">
              <button
                onClick={() => router.push('/')}
                className="font-medium text-teal-600 border-b-2 border-teal-600 pb-1 text-sm"
              >
                {t.nav_home}
              </button>
              ...
              <select
                ...
                aria-label="Language"
                className="bg-slate-50 border border-slate-200 text-xs rounded-lg p-1.5 focus:outline-none focus:border-teal-500 font-semibold text-slate-700 cursor-pointer"
              >
              ...
              {!userProfile ? (
                <button
                  ...
                  className="bg-slate-950 text-white px-4 py-2 rounded-lg font-medium hover:bg-teal-600 transition shadow-sm text-sm"
                >
                  🚪 {t.nav_sign_in}
                </button>
              ) : (
                <button
                  ...
                  className="flex items-center gap-2 border border-slate-200 hover:border-teal-500 bg-slate-50 px-3 py-1.5 rounded-xl transition shadow-sm"
                >
                  <div className="w-7 h-7 rounded-full bg-slate-900 text-white flex items-center justify-center font-bold text-xs">
                    {userProfile.name ? userProfile.name.charAt(0).toUpperCase() : 'U'}
                  </div>
                  <span className="text-xs font-bold text-slate-700">{t.nav_dashboard}</span>
                </button>
              )}
            </div>
            ...
            <div className="flex md:hidden items-center gap-2 shrink-0">
              <button
                ...
                className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-xl text-slate-800 shadow-sm active:scale-95"
        ...
        {mobileMenuOpen && (
          <>
            <button
              ...
              className="fixed inset-0 top-16 z-[-1] bg-slate-950/30 md:hidden"
            />
              ...
              <div className="max-w-7xl mx-auto px-4 py-3">
                <div className="mb-3 flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3">
                  ...
                  <select
                    ...
                    onChange={(e) => handleLangChange(e.target.value as 'en' | 'as' | 'hi')}
                    aria-label="Language"
                    className="max-w-[115px] rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-xs font-semibold text-slate-700 outline-none focus:border-teal-500"
                  >
                    <option value="en">English</option>
                    <option value="as">অসমীয়া</option>
                    <option value="hi">हिंदी</option>
                ...
                <div className="space-y-1">
                  <button
                    ...
                    className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold text-teal-600 hover:bg-teal-50"
                  >
                  ...
                  <a
                    href="/list-property"
                    onClick={(e) => {
                      requireLoginToList(e);
                      if (userProfile) setMobileMenuOpen(false);
                    }}
                    className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50"
                  >
                    <span>➕</span>{t.nav_list_property}
                  </a>

                  <button
                    onClick={() => { setMobileMenuOpen(false); router.push('/refer-a-host'); }}
                  ...
                  <button
                    onClick={() => { setMobileMenuOpen(false); router.push('/map'); }}
                    className="flex w-full items-center gap-3 rounded-xl bg-teal-600 px-3 py-3 text-left text-sm font-bold text-white hover:bg-teal-700"
                  >
                    <span>🗺️</span>{t.nav_explore_map}
                  </button>
                ...
                <div className="mt-3 border-t border-slate-100 pt-3">
                  {!userProfile ? (
                    <button
                      ...
                      className="flex w-full items-center justify-center rounded-xl bg-slate-950 px-4 py-3 text-sm font-black text-white shadow-sm hover:bg-teal-600"
                    >
                      🚪 {t.nav_sign_in}
                    </button>
                  ) : (
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700"
                    >
                      <span className="grid h-7 w-7 place-items-center rounded-full bg-slate-900 text-xs text-white">
                        {userProfile.name ? userProfile.name.charAt(0).toUpperCase() : 'U'}
                      </span>
                      {t.nav_dashboard}
                    </button>
