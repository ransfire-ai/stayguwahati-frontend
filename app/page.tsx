...
export default function HomePage() {
  ...
  const filteredProperties = properties.filter((property) => {
    ...
    return (
    <div className="min-h-screen bg-[#f6faf9] text-slate-900 font-sans pb-20 lg:pb-0">
      {/* Desktop navigation */}
      <nav className="sticky top-0 z-50 border-b border-teal-100/80 bg-[#f6faf9]/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 lg:px-8">
          <button onClick={() => router.push('/')} className="flex items-center gap-2.5">
            <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-teal-700 text-xl shadow-sm">🏠</span>
            <span className="text-xl font-black tracking-tight">Stay<span className="text-teal-700">Guwahati</span></span>
          </button>
          ...
          <div className="hidden items-center gap-2 lg:flex">
            ...
            <a href="/list-property" onClick={requireLoginToList} className="rounded-full px-4 py-2 text-sm font-semibold hover:bg-white">{t.nav_list_property}</a>
            <button onClick={() => router.push('/refer-a-host')} className="rounded-full px-4 py-2 text-sm font-semibold hover:bg-white">{t.nav_refer_host}</button>
            ...
            <button onClick={() => router.push('/support')} className="rounded-full px-4 py-2 text-sm font-semibold hover:bg-white">{t.nav_support}</button>
            <select value={currentLang} onChange={(e) => handleLangChange(e.target.value as 'en' | 'as' | 'hi')} className="rounded-full border border-teal-100 bg-white px-3 py-2 text-xs font-semibold">
      ...
      {/* Distinctive StayGuwahati hero */}
      <section className="relative overflow-hidden bg-[#083c3b] text-white">
        <div className="absolute -right-24 -top-24 h-72 w-72 rounded-full bg-teal-400/20 blur-3xl" />
        <div className="absolute -bottom-32 -left-20 h-80 w-80 rounded-full bg-amber-300/15 blur-3xl" />

        <div className="relative mx-auto max-w-7xl px-5 py-10 sm:py-14 lg:px-8 lg:py-16">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-teal-300/30 bg-white/10 px-4 py-2 text-xs font-bold uppercase tracking-[0.16em] text-teal-100">
              <span>✦</span> Made for Guwahati
            </div>
            <h1 className="mt-5 text-4xl font-black leading-[1.02] tracking-tight sm:text-5xl lg:text-6xl">
              Stay local.<br />
              <span className="text-amber-300">Feel Guwahati.</span>
            </h1>
            <p className="mt-5 max-w-2xl text-base leading-7 text-teal-50/80 sm:text-lg">
              Handpicked homestays, warm local hosts and neighbourhoods worth discovering —
              all in one place.
            </p>

            <button onClick={openSearch} className="mt-7 rounded-2xl bg-amber-300 px-6 py-3.5 text-sm font-black text-[#173b38] shadow-lg shadow-black/10 transition hover:-translate-y-0.5">
              Discover a stay →
            </button>
          </div>
          ...
          <div className="mt-9 grid max-w-4xl gap-3 rounded-3xl bg-white p-3 text-slate-900 shadow-2xl sm:grid-cols-[1fr_auto]">
            <div className="rounded-2xl bg-[#f2f8f6] px-4 py-3">
              ...
              <input
                ...
                placeholder="Search Guwahati neighbourhood"
                className="mt-1 w-full bg-transparent text-sm font-semibold outline-none placeholder:text-slate-400"
              />
            </div>
            <button onClick={openSearch} className="rounded-2xl bg-teal-700 px-7 py-3 text-sm font-black text-white">
              Search stays
            </button>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-5 pt-9 lg:px-8">
        <div className="flex items-end justify-between gap-4">
          ...
          <button onClick={() => router.push('/map')} className="hidden rounded-full border border-teal-200 bg-white px-4 py-2 text-xs font-bold text-teal-800 sm:block">
            Open map →
          </button>
        </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            <button
              key={locality}
              onClick={() => { handleLocalitySelect(locality); openSearch(); }}
              className="group rounded-3xl border border-teal-100 bg-white p-4 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-md"
            >
      ...
      <main id="listings" className="mx-auto max-w-7xl scroll-mt-24 px-5 pb-10 pt-10 lg:px-8">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              {filteredProperties.length} {t.active_options_text}
            </p>
          </div>
          <button onClick={() => { setSelectedFilterLocality(null); setSearchLocality(''); setSearchQuery(''); }} className="hidden rounded-full bg-white px-4 py-2 text-xs font-bold text-teal-700 shadow-sm ring-1 ring-teal-100 sm:block">
            All stays
          </button>
        </div>
          ...
          {['All', 'Uzan Bazar', 'Paltan Bazar', 'Beltola', 'Ganeshguri', 'Dispur'].map((locality) => (
            <button
              ...
              className={`shrink-0 rounded-full px-4 py-2 text-xs font-bold ${
                (!selectedFilterLocality && !searchLocality && locality === 'All') ||
        ...
        {loading ? (
          <div className="rounded-3xl border border-teal-100 bg-white py-24 text-center shadow-sm">
            ...
            <p className="mt-3 text-sm font-semibold text-slate-500">{t.pipeline_loading}</p>
          </div>
        ) : filteredProperties.length === 0 ? (
          <div className="rounded-3xl border border-teal-100 bg-white px-6 py-20 text-center shadow-sm">
            <div className="text-5xl">🌴</div>
            <p className="mt-4 text-lg font-black">{t.no_properties}</p>
            <p className="mt-2 text-sm text-slate-500">{t.no_properties_sub}</p>
            <button onClick={() => { setSelectedFilterLocality(null); setSearchLocality(''); setSearchQuery(''); }} className="mt-5 rounded-full bg-teal-700 px-5 py-2.5 text-sm font-bold text-white">
              Show all stays
            </button>
          </div>
        ) : (
                  ...
                  <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
                    <button type="button" onClick={() => handleReserveSpace(stay)} className="absolute inset-0 z-0 h-full w-full">
                      <img
                        ...
                        onError={(e) => { if (e.currentTarget.src !== FALLBACK_PROPERTY_IMAGE) e.currentTarget.src = FALLBACK_PROPERTY_IMAGE; }}
                      />
                    </button>

                    <div className="absolute inset-x-0 top-0 flex items-center justify-between p-4">
                      {isFavourite ? (
                        <span className="rounded-full bg-amber-300 px-3 py-1.5 text-[11px] font-black text-[#173b38] shadow-sm">LOCAL FAVOURITE</span>
                      ) : (
                        <span className="rounded-full bg-[#083c3b]/90 px-3 py-1.5 text-[11px] font-black text-white">STAYGUWAHATI</span>
                      )}
                      ...
                      <button
                        ...
                        className="flex h-10 w-10 items-center justify-center rounded-full bg-white/95 text-xl shadow-md"
                      >
                        {isSaved ? '♥' : '♡'}
                      </button>
                    </div>
                  ...
                  <button type="button" onClick={() => handleReserveSpace(stay)} className="block w-full p-5 text-left">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <h3 className="truncate text-lg font-black text-slate-900">{stay.title || 'Guwahati homestay'}</h3>
                        <p className="mt-1 text-sm font-semibold text-teal-700">{stay.locality || 'Guwahati'}, Assam</p>
                      </div>
                      {rating !== null && (
                        <div className="shrink-0 rounded-full bg-amber-50 px-2.5 py-1 text-xs font-black text-amber-700">
                          ★ {rating.toFixed(1)}
                        </div>
                      )}
                    </div>
                    ...
                    <p className="mt-3 line-clamp-2 text-sm leading-5 text-slate-500">
                      {stay.description || 'A comfortable local stay with a warm Guwahati welcome.'}
                    </p>

                    <div className="mt-4 flex items-end justify-between border-t border-slate-100 pt-4">
                      <div>
                        <span className="text-lg font-black text-slate-900">₹{Number(stay.pricePerNight || 0).toLocaleString('en-IN')}</span>
      ...
      <section className="mx-auto mb-10 max-w-7xl px-5 lg:px-8">
        <div className="overflow-hidden rounded-[32px] bg-[#e6f3ef] p-7 sm:p-9 lg:flex lg:items-center lg:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-teal-700">For local hosts</p>
            <h2 className="mt-2 text-2xl font-black">Your spare room could welcome someone to Guwahati.</h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">List your property on StayGuwahati and connect with guests looking for genuine local stays.</p>
          </div>
          <a href="/list-property" onClick={requireLoginToList} className="mt-5 inline-flex rounded-2xl bg-teal-700 px-6 py-3 text-sm font-black text-white lg:mt-0">
            List your property →
          </a>
        </div>
      </section>
      ...
      {/* Floating map button */}
      <button onClick={() => router.push('/map')} className="fixed bottom-24 left-1/2 z-40 -translate-x-1/2 rounded-full bg-[#083c3b] px-6 py-3.5 text-sm font-black text-white shadow-xl lg:bottom-8">
        Explore Map 🗺️
      </button>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-teal-100 bg-white/95 px-5 pb-[max(10px,env(safe-area-inset-bottom))] pt-2 backdrop-blur lg:hidden">
        <div className="mx-auto flex max-w-md items-center justify-around">
          <button onClick={openSearch} className="flex min-w-20 flex-col items-center gap-1 py-1 text-teal-700">
            <span className="text-2xl">⌕</span><span className="text-xs font-black">Explore</span>
          </button>
          <button onClick={() => router.push('/wishlist')} className="flex min-w-20 flex-col items-center gap-1 py-1 text-slate-500">
            <span className="text-2xl">♡</span><span className="text-xs font-semibold">Saved</span>
          </button>
          <button onClick={() => router.push(userProfile ? '/dashboard' : '/login')} className="flex min-w-20 flex-col items-center gap-1 py-1 text-slate-500">
            <span className="text-2xl">◯</span><span className="text-xs font-semibold">{userProfile ? 'Profile' : 'Sign in'}</span>
          </button>
        </div>
      </nav>

      <footer className="border-t border-teal-100 bg-white px-5 py-8 text-center text-xs text-slate-500">
        © 2026 StayGuwahati · Local stays, local stories.
      </footer>
    </div>
  );
}
