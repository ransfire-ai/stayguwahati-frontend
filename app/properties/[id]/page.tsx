...
const API_BASE_URL = 'https://stayguwahati-backend.onrender.com';
...
function getHostId(property: Property): string {
  const host = typeof property.host === 'string' ? {} : property.host || {};
  return String(host.hostId || host._id || host.id || '').trim();
}
...
function PropertyDetailsContent() {
  ...
  useEffect(() => {
    ...
    async function loadProperty() {
      ...
      // 2. Only if the exact endpoint fails, use the listings endpoint and
      // locate the SAME MongoDB _id. Never substitute another property.
      if (!loaded) {
        try {
      ...
      <div className="mx-auto max-w-[1440px] px-4 py-5 sm:px-6 lg:px-8">
        ...
        <section className="mt-8 space-y-5 md:hidden">
          ...
          <div className="rounded-[28px] border border-[#d9e0db] bg-white p-5">
            ...
            <h2 className="mt-2 text-lg font-black">Meet your host</h2>
            {getHostId(property) ? (
              <Link
                href={`/host-profile?id=${encodeURIComponent(getHostId(property))}`}
                className="mt-4 flex items-center gap-3 rounded-2xl bg-[#f4f7f4] p-3"
              >
                {derived.avatar ? (
                  <img src={derived.avatar} alt={derived.hostName} className="h-14 w-14 rounded-xl object-cover" />
                ) : (
                  <div className="grid h-14 w-14 place-items-center rounded-xl bg-[#dcece4] text-[#28655c]">
                    <UserRound />
                  </div>
                )}
                <div className="min-w-0 flex-1">
                  <p className="font-black">{derived.hostName}</p>
                  <p className="mt-1 text-xs text-[#71827d]">Local host on StayGuwahati</p>
                </div>
                <span className="text-[11px] font-black text-[#28655c]">View →</span>
              </Link>
            ) : (
              <div className="mt-4 rounded-2xl bg-[#f4f7f4] p-3 text-xs font-bold text-[#9aa7a2]">
                Host profile unavailable until the backend supplies the host ID.
              </div>
            )}
          </div>
            ...
            <section className="rounded-[28px] border border-[#d9e0db] bg-white p-6 sm:p-8">
              ...
              <div className="mt-6 flex flex-col gap-5 rounded-3xl bg-[#f4f7f4] p-5 sm:flex-row sm:items-center">
                ...
                {getHostId(property) ? (
                  <Link
                    href={`/host-profile?id=${encodeURIComponent(getHostId(property))}`}
                    className="inline-flex items-center gap-2 text-xs font-bold text-[#28655c]"
                  >
                    <UserRound className="h-4 w-4" />
                    Host profile
                  </Link>
                ) : (
                  <span className="inline-flex items-center gap-2 text-xs font-bold text-[#9aa7a2]">
                    <UserRound className="h-4 w-4" />
