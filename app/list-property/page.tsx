...
export default function ListPropertyPage() {
  ...
  const steps = [
    ...
    { number: 2, title: 'Room types & pricing', hint: 'Units, guests & rates' },
    { number: 3, title: 'Make it yours', hint: 'Amenities & photos' },
    { number: 4, title: 'Pin the place', hint: 'Exact location' },
    { number: 5, title: 'Meet the host', hint: 'Review & publish' },
  ];
  ...
  const updateRoomType = <K extends keyof RoomTypeDraft>(index: number, key: K, value: RoomTypeDraft[K]) => {
    setRoomTypes((current) => current.map((room, i) => (i === index ? { ...room, [key]: value } : room)));
  };
  ...
  const roomInventoryTotal = roomTypes.reduce((sum, room) => sum + Math.max(0, Number(room.units) || 0), 0);
  const roomStartingPrice = roomTypes.length
    ? Math.min(...roomTypes.map((room) => Number(room.pricePerNight) || Infinity))
    : 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (roomTypes.length === 0) {
      alert('Please add at least one room type with its inventory, maximum guests and nightly price.');
      setStep(2);
      return;
    }

    const invalidRoom = roomTypes.find((room) =>
      !room.name.trim() ||
      Number(room.units) < 1 ||
      Number(room.maxGuests) < 1 ||
      Number(room.pricePerNight) <= 0
    );
    if (invalidRoom) {
      alert('Please complete every room type before publishing.');
      setStep(2);
      return;
    }

    const parsedPrice = roomStartingPrice > 0 && Number.isFinite(roomStartingPrice)
      ? roomStartingPrice
      : Number(price);
    if (!Number.isFinite(parsedPrice) || parsedPrice <= 0) {
      alert('Please add a valid room price.');
      setStep(2);
      return;
    }
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-7 sm:py-10">
          <section className="rounded-[32px] bg-gradient-to-br from-[#103b36] via-[#173f3a] to-[#356b62] text-white px-6 py-8 sm:px-10 sm:py-12 shadow-[0_20px_55px_rgba(23,63,58,.16)]">
            ...
            <div className="mt-5 grid gap-5 lg:grid-cols-[1.3fr_.7fr] lg:items-end">
              ...
              <div className="rounded-2xl border border-white/15 bg-black/10 p-4">
                <p className="text-xs text-[#cfe0da]">Step {step} of 5</p>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-[#2d756b]/20 focus:border-[#2d756b] font-medium text-gray-900"
                />
                <p className="mt-1 text-[11px] text-gray-400">For multiple room types, the lowest room rate is used as the property's starting price.</p>
              </div>
            </div>
            ...
            {step === 2 && <div className="space-y-5">
              <div className="rounded-2xl border border-[#cfe0d8] bg-[#f4f8f5] p-4 sm:p-5">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div>
                    <p className="text-sm font-black text-[#173f3a]">Room types / units & pricing</p>
                    <p className="mt-1 text-xs text-[#71817c]">
                      Add each room category separately. Set its inventory, maximum guests and nightly rate.
                    </p>
                  </div>
              ...
              {roomTypes.length === 0 && (
                <div className="rounded-2xl border border-dashed border-[#b8d2c8] bg-white p-6 text-center">
                  <div className="text-3xl">🛏️</div>
                  <p className="mt-2 text-sm font-black text-gray-900">Add your first room type</p>
                  <p className="mt-1 text-xs text-gray-500">For example: Budget Room, Deluxe Room or Private 1 BHK.</p>
                </div>
              )}

              <div className="space-y-4">
                {roomTypes.map((room, index) => (
                  <div key={index} className="rounded-2xl border border-[#d6dfd9] bg-white p-4 sm:p-5 shadow-sm">
                    <div className="flex items-center justify-between gap-3 mb-4">
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-[.16em] text-[#3f776e]">Room type {index + 1}</p>
                        <p className="mt-1 text-sm font-black text-[#1f3431]">Unit & pricing details</p>
                      </div>
                      <button type="button" onClick={() => removeRoomType(index)} className="rounded-lg border border-rose-200 px-3 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50">Remove</button>
                    </div>
              ...
              <button type="button" onClick={addRoomType} className="w-full rounded-xl border-2 border-dashed border-[#8eb9aa] bg-[#f4f8f5] hover:bg-[#e7f0eb] px-4 py-3 text-sm font-black text-[#28655c] transition">
                + Add another room type
              </button>

              {roomTypes.length > 0 && roomStartingPrice !== Infinity && (
                <div className="rounded-xl bg-[#173f3a] px-4 py-3 text-sm text-white flex items-center justify-between gap-3">
                  <span className="font-semibold">Starting price</span>
                  <span className="font-black">₹{roomStartingPrice.toLocaleString('en-IN')} / night</span>
                </div>
              )}
            </div>}
